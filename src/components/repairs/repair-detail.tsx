...
export function RepairDetail({ job, communicationLogs = [], partsList = [], backTo, settings = { hourlyRate: 42.50, defaultMarkup: 25, defaultTax: 21 }, allTags = [], repairTags = [], customerRepairs = [], users = [], allCustomers = [], tasks = [], partRequests = [], repairWorkers = [], activeUsers = [], findings = [], blockers = [], estimateLines = [], dismissedWorkshopItems: initialDismissed = [], partCategories = [], photos = [], timeEntries = [], activeTimers = [], syncState = null, garageActivity = [], canLinkHoldedDocuments = false }: RepairDetailProps) {
  ...
  return (
    <div className="min-h-screen bg-background animate-fade-in">
      ...
      {/*
        Two-column shell on lg+. Main column keeps the existing content
        flow; the right rail surfaces the most-needed context (status,
        customer, unit, totals, jump-links) so the user doesn't have to
        scroll to find them. On smaller screens the rail collapses to
        the top of the page above the main flow — same visual order on
        a phone, side-by-side on a desktop.
      */}
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-7">
      <div className="min-w-0 space-y-6 sm:space-y-8 lg:order-1">
      ...
      {/* ── Sticky right rail ───────────────────────────────────────
          Compact contextual sidebar. Stays in view as the user scrolls
          the long detail editor on the left. Pure read-only summary +
          jump-links into the existing sections — no duplicated state. */}
      <aside className="lg:order-2 lg:sticky lg:top-[88px] lg:self-start print:hidden">
        <RepairSidebarRail
          job={job}
          status={status}
          priority={priority}
          customerName={job.customer?.name}
          unitLabel={job.unit ? [job.unit.year, job.unit.brand, job.unit.model].filter(Boolean).join(" ") : null}
          unitPlate={job.unit?.licensePlate}
          locationName={job.location?.name}
          totalInclTax={costLinesTotalInclTax}
          partsCount={partRequests.length}
          partsPending={partRequests.filter((p) => !["received", "cancelled"].includes(p.status)).length}
          tasksCount={tasks.length}
          tasksDone={tasks.filter((t) => t.status === "done").length}
          photosCount={photos.length}
          eventsCount={job.events?.length ?? 0}
        />
      </aside>
...
// ─── Right rail: at-a-glance summary + section jump-links ───
function RepairSidebarRail({
  job,
  status,
  priority,
  customerName,
  ...
  job: { id: string; publicCode: string | null; customer?: { id: string; name?: string | null } | null; unit?: { id: string } | null };
  status: string;
  priority: string;
  ...
  const tasksPct = tasksCount > 0 ? Math.round((tasksDone / tasksCount) * 100) : 0;

  return (
    <div className="space-y-3">
      {/* Status / Priority */}
      <div className="rounded-2xl border border-border/60 bg-card p-4 shadow-[0_1px_2px_0_rgba(0,0,0,0.03)]">
        <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">Status</p>
        <span
          className={cn(
            "inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold",
            STATUS_COLORS[status as RepairStatus] ?? "bg-muted text-muted-foreground",
          )}
        >
        ...
        <div className="mt-3 flex items-center justify-between gap-2 border-t border-border/50 pt-3">
          ...
          <span
            className={cn(
              ...
              priority === "urgent" && "bg-red-50 text-red-700 dark:bg-red-500/15 dark:text-red-300",
              priority === "high" && "bg-orange-50 text-orange-700 dark:bg-orange-500/15 dark:text-orange-300",
              priority === "normal" && "bg-foreground/[0.06] text-foreground/80",
      ...
      <div className="rounded-2xl border border-border/60 bg-card p-4 shadow-[0_1px_2px_0_rgba(0,0,0,0.03)]">
        {customerName ? (
          <Link
            href={job.customer?.id ? `/customers/${job.customer.id}` : "#"}
            className="group block rounded-lg -mx-1 px-1 py-1 transition-colors hover:bg-muted"
          >
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">Customer</p>
            <p className="mt-0.5 truncate text-sm font-medium tracking-[-0.005em] text-foreground transition-transform group-hover:translate-x-0.5">
              {customerName}
            </p>
          </Link>
        ) : (
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">Customer</p>
            <p className="mt-0.5 text-sm italic text-muted-foreground">Not linked</p>
          </div>
        )}

        {(unitLabel || unitPlate) && (
          <div className="mt-3 border-t border-border/50 pt-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">Unit</p>
            <p className="mt-0.5 truncate text-sm font-medium tracking-[-0.005em] text-foreground">{unitLabel || "—"}</p>
            {unitPlate && (
              <p className="mt-0.5 font-mono text-[11px] uppercase tracking-wider text-muted-foreground">{unitPlate}</p>
            )}
          </div>
        )}
        ...
        {locationName && (
          <div className="mt-3 border-t border-border/50 pt-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">Location</p>
            <p className="mt-0.5 truncate text-sm font-medium tracking-[-0.005em] text-foreground">{locationName}</p>
          </div>
        )}
      </div>

      {/* At-a-glance metrics with jump-links to the relevant section */}
      <div className="overflow-hidden rounded-2xl border border-border/60 bg-card shadow-[0_1px_2px_0_rgba(0,0,0,0.03)]">
        <div className="border-b border-border/50 px-4 py-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/70">Overview</p>
          <p className="mt-1 font-mono text-2xl font-semibold tabular-nums tracking-[-0.02em] text-foreground">
        ...
        <ul className="divide-y divide-border/50 text-[12px]">
          <RailRow
            label="Tasks"
            value={tasksCount > 0 ? `${tasksDone}/${tasksCount}` : "—"}
            hint={tasksCount > 0 ? `${tasksPct}%` : undefined}
            onClick={() => scrollTo("repair-financial")}
          />
          <RailRow
            label="Parts"
            value={partsCount > 0 ? String(partsCount) : "—"}
            hint={partsPending > 0 ? `${partsPending} pending` : undefined}
            tone={partsPending > 0 ? "amber" : "default"}
            onClick={() => scrollTo("repair-financial")}
          />
          <RailRow
            label="Photos"
            ...
            onClick={() => scrollTo("repair-financial")}
          />
          <RailRow label="Events" value={eventsCount > 0 ? String(eventsCount) : "—"} />
        </ul>
      </div>

      <p className="px-1 text-[10px] text-muted-foreground/60">
        Tip: alle wijzigingen worden automatisch opgeslagen.
      </p>
    </div>
  );
}
...
function RailRow({
  ...
  const Wrap = onClick ? "button" : ("div" as const);
  return (
    <Wrap
