        ...
        {quickButtons.map((btn) => {
          ...
          return (
            <Link key={btn.label} href={isActive ? "/repairs" : href}>
              <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs transition-colors cursor-pointer ${
                isActive
                  ? "bg-foreground/[0.07] text-foreground font-medium"
                  : "text-muted-foreground/70 hover:text-muted-foreground hover:bg-muted/40"
              }`}>
                {btn.label}
                <span className="tabular-nums">{btn.value}</span>
              </span>
            </Link>
          );
        })}
